...
public class OVRGazePointer : MonoBehaviour {
    ...
    { 
        ...
        get
        {
            if (_instance == null)
            {
                Debug.Log(string.Format("Instantiating GazePointer", 0));
                _instance = (OVRGazePointer)GameObject.Instantiate((OVRGazePointer)Resources.Load("GazePointerRing", typeof(OVRGazePointer)));
            }
            return _instance;
        }
	...
	void Update () 
    {
        ...
        transform.position = cameraRig.transform.position + cameraRig.transform.forward * depth;
    ...
    public void SetPosition(Vector3 pos, Vector3 normal)
    {
        ...
        newRot.SetLookRotation(normal, cameraRig.transform.up);
        transform.rotation = newRot;

        // record depth so that distance doesn't pop when pointer leaves an object
        depth = (cameraRig.transform.position - pos).magnitude;
    ...
    {
        SetPosition(pos, cameraRig.transform.forward);
    }
    ...
    void LateUpdate()
    {
        ...
        if (positionSetsThisFrame == 0)
        {
            ...
            Quaternion newRot = transform.rotation;
            newRot.SetLookRotation(cameraRig.transform.forward, cameraRig.transform.up);
            transform.rotation = newRot;
        }
