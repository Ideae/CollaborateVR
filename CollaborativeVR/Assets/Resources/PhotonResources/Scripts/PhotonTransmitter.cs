...
public class PhotonTransmitter : PunBehaviour
{

  private static readonly string LOG_PREFIX = "[" + typeof(PhotonTransmitter).Name + "]: ";

  public const int RELIABLE_SEQUENCED_CHANNEL = 0;//MyNetworkManager.CHANNEL_RELIABLE_SEQUENCED;
  ...
  //[Server]
  public void SendBytesToClients(int transmissionId, byte[] data, PhotonPlayer player)
  {
    Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
    StartCoroutine(SendBytesToClientsRoutine(transmissionId, data, player));
  }

  //[Server]
  public IEnumerator SendBytesToClientsRoutine(int transmissionId, byte[] data, PhotonPlayer player)
  {
    Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
    ...
    photonView.RPC("RpcPrepareToReceiveBytes", player, new object[] { transmissionId, data.Length });
    yield return null;
    ...
    while (dataToTransmit.curDataIndex < dataToTransmit.data.Length - 1)
    {
      ...
      photonView.RPC("RpcReceiveBytes", player, new object[] { transmissionId, buffer });
